...
import { deleteProduct, getMyProducts, fromWei } from '@/services/blockchain'
...
const Products = () => {
  ...
  const statsData = useMemo(() => {
    if (!products || !address) return []

    const myProducts = products.filter(
      (p) => p.seller?.toLowerCase() === address?.toLowerCase() && !p.deleted
    )
    const totalRevenue = myProducts.reduce((acc, p) => acc + Number(fromWei(p.price.toString())), 0)
    ...
    return [
      ...
      },
      {
        ...
        value: myProducts.filter((p) => !p.deleted && !p.soldout).length,
        subtitle: 'Products',
        trend: '+5.2%',
        trendDirection: 'up',
        icon: FiTrendingUp,
      ...
      {
        title: 'Low Stock',
        value: myProducts.filter((p) => !p.deleted && p.stock < 10).length,
        subtitle: 'Items',
        trend: '-2.3%',
        trendDirection: 'down',
        icon: FiAlertCircle,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <Link
              href="/dashboard/user/products/create"
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div
                      className={`w-10 h-10 rounded-xl ${stat.iconClass} 
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(e) =>
                      setSelectedFilters((prev) => ({ ...prev, stock: e.target.value }))
                    }
                    className="appearance-none pl-4 pr-10 py-3 bg-[#13141b] border border-gray-800/50 rounded-xl
                      text-white focus:ring-2 focus:ring-indigo-500/50 focus:border-transparent
                      cursor-pointer hover:bg-[#1c1d25] transition-colors"
                ...
                >
                  ...
                  <div className="aspect-[4/3] relative overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link
                        href={`/dashboard/user/products/edit/${product.id}`}
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-2">
                      ...
                      {product.stock < 10 && (
                        <span
                          className="px-2.5 py-1 rounded-lg text-xs font-medium bg-yellow-500/20 
...
export default withUserLayout(Products)
